...
#define API_KEY "AIzaSyCQOPaLIyCOJI71ASLu-DvJTlsbGsqRupA" // for the firebase
...
/// --------- firebase ------------
String readFileAndStoreString() {
  File jsonFile = SPIFFS.open(path, "r"); // Open the JSON file for reading
  ...
  if (jsonFile) {
    ...
    jsonFile.readBytes(buffer.get(), fileSize);

    // Close the file
    jsonFile.close();

    // Create a String from the buffer
    jsonString = String(buffer.get());

    // Now, you have the JSON content in the jsonString variable
...
void uploadFile() {
  ...
  HTTPClient http;
  String url = "https://firebasestorage.googleapis.com/v0/b/" + String(storageBucket) + "/o" + String(FilePath) +
               "?key=" + API_KEY;
  http.begin(url);
  ...
  jsonContent = readFileAndStoreString();
  Serial.println("\n");

  Serial.println(jsonContect);
  Serial.println("\n");
